...
include_once "classes/class.Banner_Horizontal.php";
$objBannerHorizontal = new BannerHorizontal();

include_once "classes/class.Crop_Imagem.php";

$permissao = $objSecao->permissaoSecaoFixaUsuario("67",$objSession2->get('tlAdmLoginId'));

//verifica qual a ação está sendo solicitada pela câmada de visão(formulários)
switch ($objPost->param['acao']) {
case "frmCad":
	// inclui o arquivo
	$abrePag = "../frms/frmCadBanner_Horizontal.php";
	break;
case "cadastrar":
	...
	if($objPost->param["imagem"]["name"] !=""){
		...
		$dir = "arq_banner_horizontal/";
		if(!file_exists("arq_banner_horizontal")) {
				$objUteis->criaDir("arq_banner_horizontal");
		}
		$nomeImg = "banner_horizontal".time().".".$formatoImgDestaque;
		$temp = $dir.$nomeImg;
	...
	$result = $objBannerHorizontal->cadastrar($form);
	...
	$objUteis
			->showResult("Cadastrado com sucesso.",
					...
					"mostraMensagem",
					'index.php?acao=listar&ctrl=banner_horizontal');
	exit();
	break;
case "listar":
	// lista todos os dados do banco de dados
	$condicao = array();
	$banners = $objBannerHorizontal->listar();
	$objUteis->encode($banners);
	// inclui o formulario
	$abrePag = "../frms/listaBanner_Horizontal.php";
	break;
case "frmAlterar":
	...
	$bannerForm = $objBannerHorizontal->lista($condicao);
	$objUteis->encode($bannerForm);
	// inclui o formulario
	$abrePag = "../frms/frmAltBanner_Horizontal.php";
	break;
case "alterar":
	...
	$objUteis->decode($form);
	$result = $objBannerHorizontal->alterar($form);
...
case "deletar":
	...
	$result = $objBannerHorizontal->deletar($objPost->param['id']);
...
case "publicar":
	//publica o objeto
	$result = $objBannerHorizontal->publicar($objPost->param['id'], $objPost->param['status']);
