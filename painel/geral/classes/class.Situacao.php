...
class Situacao {
	...
	function cadastrar($form) {
		...
		$result = $sqlGl->DBInsertData("imoveis_situacao($campos)", $valores);
	...
	function alterar($form) {
		...
		$result = $sqlGl->DBUpdate("imoveis_situacao", $params, "id=" . $form->id);
	...
	function deletar($id) {
		...
		$result = $sqlGl->DBDeleteData("imoveis_situacao", "WHERE id='" . $id . "'");
	...
	function listar($atributos=array()) {
		...
		$result = $sqlGl -> DBSelectGen('
			SELECT * FROM imoveis_situacao WHERE 1 '.$condicao.'
		');
	...
	function verifica_relacao_imovel_situacao($atributos=array()) {
		//chamada ao objeto da classe de abstra��o de banco de dados
		global $sqlGl;
		...
		//$result = $sqlGl -> DBSelectGen("SELECT * FROM imoveis WHERE 1 $condicao ORDER BY $orderBy $limite");
		$result = $sqlGl -> DBSelectGen('
			SELECT imoveis_situacao_id FROM imoveis WHERE 1 '.$condicao.'
		');
	...
	function publicar($id, $status) {
		...
		$result = $sqlGl
				->DBUpdate("imoveis_situacao", "status='" . $status . "'",
						"id='" . $id . "'");
	...
	function alteraOrdem($id, $posicao) {
		global $sqlGl;
		$result = $sqlGl
				->DBUpdate("imoveis_situacao", "ordem='" . $posicao . "'", "id="
						. $id);
		//fecha a conexão
		$sqlGl->DBClose();
		return $result;
