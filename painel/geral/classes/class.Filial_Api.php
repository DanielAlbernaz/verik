...
class FilialApi {	
	...
	function cadastrar($form) {
		...
		$result = $sqlGlEx->insertInto('filiais',$form);
	...
	function alterar($form) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGlEx;
		$result = $sqlGlEx->update('filiais')->set($form)->where('id', $form['id']);
	...
	function deletar($id) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGlEx;
		$result = $sqlGlEx->deleteFrom("filiais")->where("id",$id);
	...
	function listar($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGlEx -> from("filiais")->where($atributos)->orderBy($orderBy);
	...
	function listarfiliais($parametros) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGlEx;
	...
	function listaCaracteristicas($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGlEx -> from("caracteristica_filiais")->where($atributos)->orderBy($orderBy);
		$aValores = $aValores->fetchAll();
		$aValores['num'] = count($aValores);
	...
	function listaFotos($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGlEx -> from("filiais_fotos")->where($atributos)->orderBy($orderBy);
		$aValores = $aValores->fetchAll();
		$aValores['num'] = count($aValores);
	...
	function publicar($id, $status) {
		global $sqlGlEx;
		$result = $sqlGlEx->update('filiais')->set('status',$status)->where('id', $id);
		$result = $result->execute(true);
	...
	function pegarListagensParaAlterarBack($fromPosition, $toPosition) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGlEx;
		$aValores = $sqlGlEx -> from("filiais")->where('ordem <= :ordem1 && ordem >= :ordem2 ',array(':ordem1' => $fromPosition, ':ordem2' => $toPosition));
		$aValores = $aValores->fetchAll();
		$aValores['num'] = count($aValores);
	...
	function alteraOrdem($id, $posicao) {
		global $sqlGlEx;
		$result = $sqlGlEx->update('filiais')->set('ordem',$posicao)->where('id', $id);
		$result = $result->execute(true);
	...
	function proximo_id() {
	    ...
	    $stmt = $pdo->query("SHOW TABLE STATUS LIKE 'filiais'");
	...
	function cadastrarFoto($form) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGlEx;
		$result = $sqlGlEx->insertInto('filiais_fotos',$form);
		$lastInsert = $result->execute();
	...
	function cadastrarCaracteristica($form) {
		...
		$result = $sqlGlEx->insertInto('caracteristica_filiais',$form);
		$lastInsert = $result->execute();
		//retorna o resultado da query para a câmada de controle
	...
	function listarCategorias($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGlEx -> from("categoria_filiais")->where($atributos)->orderBy($orderBy);
		$aValores = $aValores->fetchAll();
	...
	function listar_fotos($atributos=array()) {
	    ...
	    $sql = 'SELECT
	                nft.id,
		        	nft.img
                FROM filiais_fotos nft
                WHERE nft.id_filiais = :IDfiliais';
	    
	    $stmte = $sqlGlEx->pdo->prepare($sql);
	    $stmte->bindValue('IDfiliais',  $atributos['id'], PDO::PARAM_STR);
	    if($stmte->execute()) {
	        $aValores = $stmte->fetchAll(PDO::FETCH_OBJ);
	        $aValores['num'] = $stmte->rowCount();
	        return $aValores;
	...
	function deletarFoto($id) {
	    ...
	    //chamada ao objeto da classe de abstração de banco de dados
	    global $sqlGlEx;
	    $result = $sqlGlEx->deleteFrom("filiais_fotos")->where("id",$id);
	    $result = $result->execute();
	...
	function listarfiliaisRelacionados($atributos=array(),$orderBy=null,$limit=null) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGlEx;
                $sql = 'SELECT ';
                $sql .= '   * ';
	...
	function listarfiliaisRelacionadosfiliais($atributos=array(),$orderBy=null,$limit=null) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGlEx;
                $sql = 'SELECT ';
                $sql .= '   * ';
	...
	function deletarHabilidade($id) {
		...
		$result = $sqlGlEx->deleteFrom("caracteristica_filiais")->where("id_filiais",$id);
    ...
    function listarMarcasComfiliais($atributos=array(),$orderBy=null) {
        global $sqlGlEx;
	...
	function listarfiliaisNoCarrinho($atributos=array(),$orderBy=null) {
        global $sqlGlEx;
