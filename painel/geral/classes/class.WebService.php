...
class WebService {
    ...
    public function inserirImovel($imovel) {
		echo 'aqui';
		echo 'Line ' . __LINE__ . ' Method ' . __METHOD__ . '<br/>';		
...
//                     print 'n�o foi possivel salvar a imagem';
...
//             print_r($retorno['erro']) . '<hr />';
        ...
        $retorno = $this->sqlExcluirImovelFoto($idImovel);
...
//                     print 'n�o foi possivel salvar a imagem';
    ...
    public function excluirImovel($codigoImovel) {
        $bRetorno = true;

        $retorno = $this->sqlExcluirImovelFoto($codigoImovel);
        ...
        $retorno = $this->sqlExcluirImovel($codigoImovel);
	...
	private function sqlInserirImovel($imovel) {
		...
		$stmte->bindValue('CATEGORIA',        $imovel['categoria'],        PDO::PARAM_STR);
...
// 	        print 'atualizado - ' . $imovel['id_imovel'] . '<hr />';
	...
	public function sqlDadosImovel($codigoImovel) {
	    ...
	    $stmte->bindValue('CODIGO_IMOVEL', $codigoImovel, PDO::PARAM_INT);
	...
	public function sqlExcluirImovel($codigoImovel) {

		$sql = 'DELETE FROM imovel WHERE codigo_imovel = :CODIGO_IMOVEL';

		$stmte = $this->conn->prepare($sql);

		$stmte->bindValue('CODIGO_IMOVEL', $codigoImovel, PDO::PARAM_STR);
	...
	private function sqlExcluirImovelFoto($idImovel) {

	    $sql = 'DELETE FROM imovel_foto WHERE id_imovel = :ID_IMOVEL';

	    $stmte = $this->conn->prepare($sql);

	    $stmte->bindValue('ID_IMOVEL', $idImovel, PDO::PARAM_STR);
	...
	private function excluirImovelFoto($idImovel) {
	    ...
	    if (file_exists($dirImovel) && rmdir($dirImovel)) {
