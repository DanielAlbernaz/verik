...
class Missao {
	...
	function cadastrar($form) {
		...
		$result = $sqlGl->insertInto('missao',$form);
	...
	function alterar($form) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->update('missao')->set($form)->where('id', $form['id']);
	...
	function deletar($id) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->deleteFrom("missao")->where("id",$id);
	...
	function listar($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGl -> from("missao")->where($atributos)->orderBy($orderBy);
	...
	function mostraNome($id) {
		...
		$aValores = $sqlGl -> from("missao")->where('id',$id);
		$aValores = $aValores->fetch();
	...
	function publicar($id, $status) {
		global $sqlGl;
		$result = $sqlGl->update('missao')->set('status',$status)->where('id', $id);
		$result = $result->execute(true);
	...
	function pegarListagensParaAlterarBack($fromPosition, $toPosition) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$aValores = $sqlGl -> from("missao")->where('ordem <= :ordem1 && ordem >= :ordem2 ',array(':ordem1' => $fromPosition, ':ordem2' => $toPosition));
		$aValores = $aValores->fetchAll();
		$aValores['num'] = count($aValores);
	...
	function alteraOrdem($id, $posicao) {
		global $sqlGl;
		$result = $sqlGl->update('missao')->set('ordem',$posicao)->where('id', $id);
		$result = $result->execute(true);
