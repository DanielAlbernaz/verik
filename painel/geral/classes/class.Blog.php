...
class Blog {
	...
	function cadastrar($form) {
		...
		$result = $sqlGl->insertInto('blog',$form);
	...
	function alterar($form) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->update('blog')->set($form)->where('id', $form['id']);
	...
	function deletar($id) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->deleteFrom("blog")->where("id",$id);
	...
	function listar($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGl -> from("blog")->where($atributos)->orderBy($orderBy);
	...
	function listarBlog($parametros) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
	...
	function publicar($id, $status) {
		global $sqlGl;
		$result = $sqlGl->update('blog')->set('status',$status)->where('id', $id);
		$result = $result->execute(true);
	...
	function pegarListagensParaAlterarBack($fromPosition, $toPosition) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$aValores = $sqlGl -> from("blog")->where('ordem <= :ordem1 && ordem >= :ordem2 ',array(':ordem1' => $fromPosition, ':ordem2' => $toPosition));
		$aValores = $aValores->fetchAll();
		$aValores['num'] = count($aValores);
	...
	function alteraOrdem($id, $posicao) {
		global $sqlGl;
		$result = $sqlGl->update('blog')->set('ordem',$posicao)->where('id', $id);
		$result = $result->execute(true);
	...
	function proximo_id() {
	    ...
	    $stmt = $pdo->query("SHOW TABLE STATUS LIKE 'blog'");
	...
	function cadastrarFoto($form) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->insertInto('blog_fotos',$form);
		$lastInsert = $result->execute();
	...
	function listarCategorias($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGl -> from("categoria_blog")->where($atributos)->orderBy($orderBy);
		$aValores = $aValores->fetchAll();
	...
	function listar_fotos($atributos=array()) {
	    ...
	    $sql = 'SELECT
		        	...
		        	nft.img
                FROM blog_fotos nft
                WHERE nft.id_blog = :IDBLOG';
	    
	    $stmte = $sqlGl->pdo->prepare($sql);
	    $stmte->bindValue('IDBLOG',  $atributos['id'], PDO::PARAM_STR);
	    if($stmte->execute()) {
	        $aValores = $stmte->fetchAll(PDO::FETCH_OBJ);
	        $aValores['num'] = $stmte->rowCount();
	        return $aValores;
	...
	function deletarFoto($id) {
	    ...
	    //chamada ao objeto da classe de abstração de banco de dados
	    global $sqlGl;
	    $result = $sqlGl->deleteFrom("blog_fotos")->where("id",$id);
	    $result = $result->execute();
	...
	function listarBlogRelacionados($atributos=array(),$orderBy=null,$limit=null) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
                $sql = 'SELECT ';
                $sql .= '   * ';
	...
	function listarBlogRelacionadosBlog($atributos=array(),$orderBy=null,$limit=null) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
                $sql = 'SELECT ';
                $sql .= '   * ';
