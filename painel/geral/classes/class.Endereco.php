...
class Endereco {
	...
	function cadastrar($form) {
		...
		$result = $sqlGl->insertInto('endereco',$form);
	...
	function alterar($form) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->update('endereco')->set($form)->where('id', $form['id']);
	...
	function deletar($id) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->deleteFrom("endereco")->where("id",$id);
	...
	function listar($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGl -> from("endereco")->where($atributos)->orderBy($orderBy);
	...
	function mostraNome($id) {
		...
		$aValores = $sqlGl -> from("endereco")->where('id',$id);
		$aValores = $aValores->fetch();
	...
	function publicar($id, $status) {
		global $sqlGl;
		$result = $sqlGl->update('endereco')->set('status',$status)->where('id', $id);
		$result = $result->execute(true);
	...
	function pegarListagensParaAlterarBack($fromPosition, $toPosition) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$aValores = $sqlGl -> from("endereco")->where('ordem <= :ordem1 && ordem >= :ordem2 ',array(':ordem1' => $fromPosition, ':ordem2' => $toPosition));
		$aValores = $aValores->fetchAll();
		$aValores['num'] = count($aValores);
	...
	function alteraOrdem($id, $posicao) {
		global $sqlGl;
		$result = $sqlGl->update('endereco')->set('ordem',$posicao)->where('id', $id);
		$result = $result->execute(true);
