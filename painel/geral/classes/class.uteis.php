...
class Uteis {
    ...
    public function enviaEmailReclamacao($emails=null, $replyTo=null, $assunto=null, $tipo=null, $anexos=null){
        ...
        switch ($tipo) {
            case 'Sugestão':
                $content = str_replace('{MENSAGEM}',        'Olá! Agradecemos a mensagem. <br/>Sua sugestão é muito importante e bem-vinda.', $content);
                break;
            case 'Reclamação':
                $content = str_replace('{MENSAGEM}',        'Olá! Agradecemos o contato. <br/> Sua opinião é muito importante para nós. <br/>Vamos analisar e tomar as medidas necessárias referentes aos seus comentários.', $content);
                break;
            case 'Dúvida':
                $content = str_replace('{MENSAGEM}',        'Olá! <br/> Agradecemos o contato. <br/>Obrigado!', $content);
                break;
            default:
                # code...
                break;
...
public function enviaEmailSugestao($emails=null, $replyTo=null, $assunto=null, $tipo=null, $anexos=null){
    ...
    function removerAcentos($string){
        $min = array('À','Á','Â','Ã','Ä','Å','Ç','È','É','Ê','Ë','Ì','Í','Î','Ï','Ñ','Ò','Ó','Ô','Õ','Ö','O','Ù','Ü','Ú','Y','à','á','â','ã','ä','å','ç','è','é','ê','ë','ì','í','î','ï','ñ','ò','ó','ô','õ','ö','ù','ü','ú','ÿ');
...
public function enviaEmailSenha($emails=null, $msg=null, $replyTo=null, $assunto=null, $anexos=null){
	...
	// $mail->SMTPDebug  = 2;
	$mail->Host = $configuracao['smtp']['host']; // EndereÃ§o do servidor SMTP
	$mail->CharSet   = 'UTF-8';                  // enable SMTP authentication
	$mail->SMTPAuth = $configuracao['smtp']['user'] && $configuracao['smtp']['senha'] ? true : false;
	if($configuracao['smtp']['user'] && $configuracao['smtp']['senha'] && $configuracao['smtp']['port']){
		$mail->Username = $configuracao['smtp']['user']; // UsuÃ¡rio do servidor SMTP
		$mail->Password = $configuracao['smtp']['senha']; // Senha do servidor SMTP
		$mail->Port = $configuracao['smtp']['port'];
	}
...
/**
 * Metodo responsavel por enviar email sem a necessidade de pegar as informações do formulário
 *
 **/
...
public function enviaEmailContatoSite($emails=null, $msg=null, $replyTo=null, $assunto=null, $anexos=null, $tituloCorpoEmail = null){
    ...
    public function calculaTempoPublicacao($data_cadastro, $data_atual) {
        ...
        if($diff->format('%m') < 1){
            if($diff->format('%d') == 1){
                return $diff->format('Publicado %d dia atrás');
            }elseif($diff->format('%d') > 1){
                return $diff->format('Publicado %d dias atrás');
            }elseif($diff->format('%H') == 1){
                return $diff->format('Publicado %H hora atrás');
            }elseif($diff->format('%H') > 1){
                return $diff->format('Publicado %H horas atrás');
            }elseif($diff->format('%H') < 1){
                return $diff->format('Publicado %i minutos atrás');
            }
        }else{
