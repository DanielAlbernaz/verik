...
class BannerMini {
	...
	function cadastrar($form) {
		...
		$result = $sqlGl->insertInto('banner_mini',$form);
	...
	function alterar($form) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->update('banner_mini')->set($form)->where('id', $form['id']);
	...
	function deletar($id) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->deleteFrom("banner_mini")->where("id",$id);
	...
	function listar($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGl -> from("banner_mini")->where($atributos)->orderBy($orderBy);
	...
	function listarbanner_mini($parametros) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
	...
	function listaCaracteristicas($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGl -> from("caracteristica_banner_mini")->where($atributos)->orderBy($orderBy);
		$aValores = $aValores->fetchAll();
		$aValores['num'] = count($aValores);
	...
	function listaFotos($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGl -> from("banner_mini_fotos")->where($atributos)->orderBy($orderBy);
		$aValores = $aValores->fetchAll();
		$aValores['num'] = count($aValores);
	...
	function publicar($id, $status) {
		global $sqlGl;
		$result = $sqlGl->update('banner_mini')->set('status',$status)->where('id', $id);
		$result = $result->execute(true);
	...
	function pegarListagensParaAlterarBack($fromPosition, $toPosition) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$aValores = $sqlGl -> from("banner_mini")->where('ordem <= :ordem1 && ordem >= :ordem2 ',array(':ordem1' => $fromPosition, ':ordem2' => $toPosition));
		$aValores = $aValores->fetchAll();
		$aValores['num'] = count($aValores);
	...
	function alteraOrdem($id, $posicao) {
		global $sqlGl;
		$result = $sqlGl->update('banner_mini')->set('ordem',$posicao)->where('id', $id);
		$result = $result->execute(true);
	...
	function proximo_id() {
	    ...
	    $stmt = $pdo->query("SHOW TABLE STATUS LIKE 'banner_mini'");
	...
	function cadastrarFoto($form) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
		$result = $sqlGl->insertInto('banner_mini_fotos',$form);
		$lastInsert = $result->execute();
	...
	function cadastrarCaracteristica($form) {
		...
		$result = $sqlGl->insertInto('caracteristica_banner_mini',$form);
		$lastInsert = $result->execute();
		//retorna o resultado da query para a câmada de controle
	...
	function listarCategorias($atributos=array(),$orderBy=null) {
		...
		$aValores = $sqlGl -> from("categoria_banner_mini")->where($atributos)->orderBy($orderBy);
		$aValores = $aValores->fetchAll();
	...
	function listar_fotos($atributos=array()) {
	    ...
	    $sql = 'SELECT
	                nft.id,
		        	nft.img
                FROM banner_mini_fotos nft
                WHERE nft.id_banner_mini = :IDbanner_mini';
	    
	    $stmte = $sqlGl->pdo->prepare($sql);
	    $stmte->bindValue('IDbanner_mini',  $atributos['id'], PDO::PARAM_STR);
	    if($stmte->execute()) {
	        $aValores = $stmte->fetchAll(PDO::FETCH_OBJ);
	        $aValores['num'] = $stmte->rowCount();
	        return $aValores;
	...
	function deletarFoto($id) {
	    ...
	    //chamada ao objeto da classe de abstração de banco de dados
	    global $sqlGl;
	    $result = $sqlGl->deleteFrom("banner_mini_fotos")->where("id",$id);
	    $result = $result->execute();
	...
	function listarbanner_miniRelacionados($atributos=array(),$orderBy=null,$limit=null) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
                $sql = 'SELECT ';
                $sql .= '   * ';
	...
	function listarbanner_miniRelacionadosbanner_mini($atributos=array(),$orderBy=null,$limit=null) {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
                $sql = 'SELECT ';
                $sql .= '   * ';
	...
	function listarUltimosbanner_mini() {
		//chamada ao objeto da classe de abstração de banco de dados
		global $sqlGl;
	...
	function deletarHabilidade($id) {
		...
		$result = $sqlGl->deleteFrom("caracteristica_banner_mini")->where("id_banner_mini",$id);
