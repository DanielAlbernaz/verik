...
include_once "classes/class.Privacidade.php";
...
case "alterar":
	...
	if($objPost->param["imagem"]["name"] !=""){
		...
		$dir = "arq_privacidade/";
		if(!file_exists("arq_privacidade")) {
				$objUteis->criaDir("arq_privacidade");
		}
		$nomeImg = "privacidade".time().".".$formatoImgDestaque;
		$temp = $dir.$nomeImg;
	...
	$objUteis
			->showResult("Alterado com sucesso",
					...
					"mostraMensagem",
					'index.php?acao=frmAlterar&ctrl=privacidade&id='.$objPost->param['id']);
	exit();
