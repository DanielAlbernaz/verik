...
include_once "classes/class.Vantagem.php";
$objVantagem = new Vantagem();

include_once "classes/class.Crop_Imagem.php";

$permissao = $objSecao->permissaoSecaoFixaUsuario("78",$objSession2->get('tlAdmLoginId'));

//verifica qual a ação está sendo solicitada pela câmada de visão(formulários)
switch ($objPost->param['acao']) {
case "frmCad":
	// inclui o arquivo
	$abrePag = "../frms/frmCadVantagem.php";
	break;
case "cadastrar":
	...
	if($objPost->param["imagem"]["name"] !=""){
		...
		$dir = "arq_vantagem/";
		if(!file_exists("arq_vantagem")) {
				$objUteis->criaDir("arq_vantagem");
		}
		$nomeImg = "vantagem".time().".".$formatoImgDestaque;
		$temp = $dir.$nomeImg;
	...
	$result = $objVantagem->cadastrar($form);
	...
	$objUteis
			->showResult("Cadastrado com sucesso.",
					...
					"mostraMensagem",
					'index.php?acao=listar&ctrl=vantagem');
	exit();
	break;
case "listar":
	// lista todos os dados do banco de dados
	$condicao = array();
	$vantagens = $objVantagem->listar();
	$objUteis->encode($vantagens);
	// inclui o formulario
	$abrePag = "../frms/listaVantagem.php";
	break;
case "frmAlterar":
	...
	$vantagemForm = $objVantagem->lista($condicao);
	$objUteis->encode($vantagemForm);
	// inclui o formulario
	$abrePag = "../frms/frmAltVantagem.php";
	break;
case "alterar":
	...
	$objUteis->decode($form);
	$result = $objVantagem->alterar($form);
...
case "deletar":
	...
	$result = $objVantagem->deletar($objPost->param['id']);
...
case "publicar":
	//publica o objeto
	$result = $objVantagem->publicar($objPost->param['id'], $objPost->param['status']);
