...
include_once "classes/class.Menu_Servico.php";
$objMenuServico = new MenuServico();
...
$permissao = $objSecao->permissaoSecaoFixaUsuario("76",$objSession2->get('tlAdmLoginId'));
...
case "frmCad":
	...
	// inclui o arquivo
	$abrePag = "../frms/frmCadMenu_Servico.php";
	break;
case "cadastrar":
	//monta o array dos post
	$form['titulo'] = $objPost->param['titulo'];
	if($objPost->param['servico']){
	...
	$result = $objMenuServico->cadastrar($form);
	...
	$objUteis
			->showResult("Cadastrado com sucesso.",
					...
					"mostraMensagem",
					'index.php?acao=listar&ctrl=menu_servico');
	exit();
	break;
case "listar":
	// lista todos os dados do banco de dados
	$condicao = array();
	$menuServicos = $objMenuServico->listar();
	$objUteis->encode($menuServicos);
	// inclui o formulario
	$abrePag = "../frms/listaMenu_Servico.php";
	break;
case "frmAlterar":
	...
	$servicoForm = $objMenuServico->lista($condicao);
	$objUteis->encode($servicoForm);
	// inclui o formulario
	$abrePag = "../frms/frmAltMenu_Servico.php";
	break;
case "alterar":
	...
	$objUteis->decode($form);
	$result = $objMenuServico->alterar($form);
...
case "deletar":
	...
	$result = $objMenuServico->deletar($objPost->param['id']);
...
case "publicar":
	//publica o objeto
	$result = $objMenuServico->publicar($objPost->param['id'], $objPost->param['status']);
