...
case "cadastrar":
	...
	$result = $objCategoriaTreinamento->cadastrar($form);
	...
	if ($result) {
		...
		$objUteis
				->gravaLog($objSession2->get('tlAdmLoginNome'),
						utf8_decode("Categoria Treinamento"),
	...
	$objUteis
			->showResult("Cadastrado com sucesso.",
					...
					"mostraMensagem",
					'index.php?acao=listar&ctrl=categoria_treinamento');
	exit();
	break;
case "listar":
	// lista todos os dados do banco de dados
	$condicao = array();
	$categoriasTreinamento = $objCategoriaTreinamento->listar();
	$objUteis->encode($categoriasTreinamento);
	// inclui o formulario
	$abrePag = "../frms/listaCategoria_Treinamento.php";
	break;
case "frmAlterar":
	...
	$categoriaForm = $objCategoriaTreinamento->lista($condicao);
	$objUteis->encode($categoriaForm);
	// inclui o formulario
	$abrePag = "../frms/frmAltCategoria_Treinamento.php";
	break;
case "alterar":
	...
	$objUteis->decode($form);
	$result = $objCategoriaTreinamento->alterar($form);
...
case "deletar":
	...
	$result = $objCategoriaTreinamento->deletar($objPost->param['id']);
...
case "publicar":
	//publica o objeto
	$result = $objCategoriaTreinamento->publicar($objPost->param['id'], $objPost->param['status']);
