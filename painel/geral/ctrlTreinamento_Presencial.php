...
$permissao = $objSecao->permissaoSecaoFixaUsuario("79",$objSession2->get('tlAdmLoginId'));

//verifica qual a ação está sendo solicitada pela câmada de visão(formulários)
switch ($_REQUEST['acao']) {
case "frmCad":
	// inclui o arquivo
	$abrePag = "../frms/frmCadTreinamento_Presencial.php";
	break;
case "cadastrar":
	...
	$result = $objTreinamentoPresencial->cadastrar($form);
	...
	if ($result) {
		...
		$objUteis
				->gravaLog($objSession2->get('tlAdmLoginNome'),
						utf8_decode("Treinamento Presencial"),
	...
	$objUteis
			->showResult("Cadastrado com sucesso.",
					...
					"mostraMensagem",
					'index.php?acao=listar&ctrl=treinamento_presencial');
	exit();
	break;
case "listar":
	// lista todos os dados do banco de dados
	$condicao = array();
	$treinamentosPresencial = $objTreinamentoPresencial->listar();
	$objUteis->encode($treinamentosPresencial);
	// inclui o formulario
	$abrePag = "../frms/listaTreinamento_Presencial.php";
	break;
case "frmAlterar":
	...
	$treinamentoPresencialForm = $objTreinamentoPresencial->lista($condicao);
	$objUteis->encode($treinamentoPresencialForm);
	// inclui o formulario
	$abrePag = "../frms/frmAltTreinamento_Presencial.php";
	break;
case "alterar":
	...
	$objUteis->decode($form);
	$result = $objTreinamentoPresencial->alterar($form);
...
case "deletar":
	...
	$result = $objTreinamentoPresencial->deletar($objPost->param['id']);
...
case "publicar":
	//publica o objeto
	$result = $objTreinamentoPresencial->publicar($objPost->param['id'], $objPost->param['status']);
