...
$acaodoformulario = 'index.php?acao=alterar&ctrl=treinamento_gravado';
...
for($i=0; $i < count($categoria); $i++){    
    if($categoria[$i]->id == $treinamentoForm->categoria ){             
        $nome_categoria = $categoria[$i]->nome_categoria;
    }   
}

$optionsCategoria[] = '<option value="" ></option>';
$optionsCategoria[] = '<option value='.$treinamentoForm->categoria.' selected="selected" >'.$nome_categoria.'</option>';
foreach($categoria as $key => $val){
    if($val->id <> NULL){
        if($val->id <> $treinamentoForm->categoria){
...
$objForm->sk_formTextUrl('Link treinamento','url','',true,'Aqui você adiciona o link do treinamento.', $treinamentoForm->url);
