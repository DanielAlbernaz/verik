...
$objForm->sk_formSelectMaior('Selecione um treinamento', 'id_treinamento_aovivo" onchange="carregarListaEmailTreinamentoAovivo(this.value); ', $optionsTreinamento, false, 'Selecione uma opção');
...
if (isset($_GET['id_treinamento_aovivo']) && !empty($_GET['id_treinamento_aovivo'])) {
    $objForm->sk_formListar('Inscritos',$dadosDaTabela,$inscritos,$campos,'lista_inscritos');    
   
echo'
    <div style="clear: both;"></div>
    <div style="float: left; margin: 10px 45px;">
      <a style="padding: 8px;" href="index.php?acao=inscricao_csv&ctrl=lista_inscritos_aovivo&id_treinamento_aovivo='.$_GET["id_treinamento_aovivo"].'" id="buttonTodos" class="button greyishB">Exportar CSV</a>
    </div>';

echo'
    <div style="float: left; margin: 10px -15px;">
      <a style="padding: 8px;" onClick="chamarEsperar();" href="index.php?acao=enviar_email&ctrl=lista_inscritos_aovivo&id_treinamento_aovivo='.$_GET["id_treinamento_aovivo"].'"  id="buttonTodos" class="button greyishB">Enviar email aos inscritos</a>
    </div>';
}
