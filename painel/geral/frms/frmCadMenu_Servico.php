...
$nomedoformulario = 'Cadastrar menu serviços';
$acaodoformulario = 'index.php?acao=cadastrar&ctrl=menu_servico';
$avisodoformulario = 'Esta página você cadastra o menu serviço.';
...
// print_rpre($servico);
$optionsServico[] = '<option value="" selected="selected"></option>';
for($i=0; $i < count($servico); $i++){
    $optionsServico[] = '<option value='.$servico[$i]->id.'>'.$servico[$i]->titulo.'</option>';
}

$objForm->sk_formSelectMaior('Exibir Serviço', 'servico', $optionsServico, false, 'Selecione uma categoria');

$objForm->sk_formText('Título','titulo','',true,'Aqui você coloca um titulo para o banner.');

$objForm->sk_formText('Link redirecionamento','url','',false,'Link de redirecionamento para o botão.');
